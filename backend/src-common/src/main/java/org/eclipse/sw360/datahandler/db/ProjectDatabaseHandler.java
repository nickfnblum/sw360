...
public class ProjectDatabaseHandler extends AttachmentAwareDatabaseHandler {
    ...
    private void addCommentToClearingRequest(Project updated, Project current, User user) {
        ...
        if (CommonUtils.isNotEmpty(added)) {
            ...
            commentText.append("\nAdded Release Ids: ").append(SW360Utils.spaceJoiner.join(releaseNames));
        }
        if (CommonUtils.isNotEmpty(removed)) {
            Set<String> releaseNames = extractReleaseNameWithId(releases, removed);
            commentText.append("\nRemoved Release Ids: ").append(SW360Utils.spaceJoiner.join(releaseNames));
        }
    ...
    private Set<String> extractReleaseNameWithId(Collection<Release> releases, Collection<String> filterCriteria) {
        return releases.stream().filter(rel -> filterCriteria.contains(rel.getId()))
                .map(rel -> new StringBuilder("<li><b>").append(SW360Utils.printFullname(rel)).append("</b> (")
                        .append(rel.getId()).append(")</li>").toString())
    ...
    private String extractReleaseNameForClearingEmail(Collection<Release> releases) {
        ...
        for (Release release : releases) {
            builder.append(BackendUtils.LIST_ITEM).append(SW360Utils.printFullname(release)).append(BackendUtils.LIST_ITEM_CLOSE);
        }
    ...
    private void sendMailForNewClearing(Project project, String projectUrl, ClearingRequest clearingRequest, User user) {
        ...
        Collection<Release> releases = CommonUtils.nullToEmptyCollection(getDirectlyLinkedReleasesInNewState(project));
        ...
        String releaseNames = extractReleaseNameForClearingEmail(releases);
        mailUtil.sendClearingMail(ClearingRequestEmailTemplate.NEW, MailConstants.SUBJECT_FOR_NEW_CLEARING_REQUEST, "", getRecipients(clearingRequest),
                ...
                String.valueOf(approvedCount), clearingRequest.getRequestedClearingDate(), releaseNames);
        if (releases.size() > 0) {
            releaseNames = new StringBuilder("Linked release(s) with clearing state new:\n").append(releaseNames).toString(); 
        }
        Comment comment = new Comment(releaseNames, user.getEmail()).setAutoGenerated(true);
    ...
    private void sendMailForUpdatedProjectWithClearingRequest(Project updated, Collection<Release> releases, User user) {
